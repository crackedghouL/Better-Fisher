    ...
    for _,state in pairs(self.States) do
        if state:NeedToRun() then
            if self.CurrentState ~= nil and self.CurrentState ~= state then
                if self.ShowOutput then print("[" .. os.date(Bot.UsedTimezone) .. "] Exit state : " .. self.CurrentState.Name) end
            ...
            if self.CurrentState ~= state then
                ...
                if self.ShowOutput then print("[" .. os.date(Bot.UsedTimezone) .. "] Enter state : " .. state.Name) end
