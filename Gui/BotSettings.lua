...
function BotSettings.DrawBotSettings()
	...
	if BotSettings.Visible then
		_, BotSettings.Visible = ImGui.Begin("Bot Settings", BotSettings.Visible, ImVec2(350, 400), -1.0)
		...
		if Bot.Settings.FishingMethod == Settings.SETTINGS_ON_NORMAL_FISHING then
			if ImGui.CollapsingHeader("NPCs options", "id_gui_npc_option", true, false) then
				if ImGui.CollapsingHeader("Vendor", "id_gui_vendor", true, false) then
					if ImGui.CollapsingHeader("Buying", "id_gui_vendor_buy", true, false) then
						...
						for key = 1,vbCount do
							...
							if value ~= nil then
								...
								if valueChanged then
									...
									if Bot.VendorState.Settings.BuyItems[key].BuyMax > 10 then
										Bot.VendorState.Settings.BuyItems[key].BuyMax = 10
									end
								end
								ImGui.NextColumn()
					...
					if ImGui.CollapsingHeader("Selling", "id_gui_vendor_sell", true, false) then
